...
class SNN_Model(nn.Module):
    ...
    def forward(self, input, hebb, win = time_window):
        c1_mem = c1_spike = torch.zeros(batch_size, cfg_cnn[0][1], 34, 34, device=device)
        c2_mem = c2_spike = torch.zeros(batch_size, cfg_cnn[1][1], 17, 17, device=device)
        c3_mem = c3_spike = torch.zeros(batch_size, cfg_cnn[2][1], 17, 17, device=device)
